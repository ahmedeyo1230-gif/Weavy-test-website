...
/**
 * BeamsBackgroundLayer — canvas beam effect as a pure absolute background.
 * Drop inside any `relative` section; add z-index above this for content.
 */
export function BeamsBackgroundLayer({
    ...
    useEffect(() => {
        ...
        function resetBeam(beam: Beam, index: number, totalBeams: number) {
            ...
            const w = container!.offsetWidth;
            const spacing = w / 3;
            beam.y = container!.offsetHeight + 100;
            beam.x = column * spacing + spacing / 2 + (Math.random() - 0.5) * spacing * 0.5;
    ...
    return (
        ...
        >
            <canvas ref={canvasRef} className="absolute inset-0" style={{ filter: "blur(15px)" }} />
            {/* Edge vignette — blends into #010709 */}
            <div
                className="absolute inset-0"
                style={{
                    background: [
                        'linear-gradient(to bottom, #010709 0%, transparent 18%, transparent 82%, #010709 100%)',
                        'linear-gradient(to right, #010709 0%, transparent 15%, transparent 85%, #010709 100%)',
                    ].join(', '),
                }}
            />
        </div>
