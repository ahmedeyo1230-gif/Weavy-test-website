...
async function screenshotHero(url, selector, path, viewport) {
  ...
  // Navigate directly to the Services hash so the Services component loads
  await page.goto(url, { waitUntil: 'networkidle2', timeout: 30000 })

  // Ensure the app reacts to the services hash by firing a hashchange event (App listens for hashchange)
  try {
    await page.evaluate(() => { window.location.hash = '#section-9'; window.dispatchEvent(new HashChangeEvent('hashchange')) })
  } catch (e) {
    // ignore
  }

  // Wait for the services list to appear and click the Graphic Design card to activate hero
  try {
    await page.waitForSelector('.service-card', { timeout: 20000 })
    // Prefer clicking the card anchor by href (deterministic)
    await page.evaluate(() => {
      const a = document.querySelector('a[href="#graphic-design-animation-detail"]')
      if (a && typeof a['click'] === 'function') { a['click'](); return }
      const cards = Array.from(document.querySelectorAll('.service-card'))
      const target = cards.find(c => (c.textContent || '').includes('Graphic Design'))
      if (target && typeof target['click'] === 'function') target['click']()
    })
    await page.waitForTimeout(800)
  } catch (e) {
    // continue even if services list isn't found
  }

  // wait for the hero container to appear after clicking
  try {
  ...
  if (box) {
    ...
    const padding = Math.min(80, Math.round(viewport.height * 0.08))
    const clip = {
...
;(async () => {
  ...
  const base = `http://localhost:${port}/#section-9`
